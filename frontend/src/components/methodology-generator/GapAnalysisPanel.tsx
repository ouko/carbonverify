...
export function GapAnalysisPanel({ gm }: { gm: GeneratedMethodology }) {
  ...
  if (!analysis) return <p className="text-surface-500 dark:text-surface-400">No gap analysis yet.</p>
  return (
    <div className="space-y-4 text-surface-700 dark:text-surface-300">
      <div className="flex items-center space-x-2">
        <span className="font-semibold text-surface-900 dark:text-surface-100">Fits existing methodology?</span>
      ...
      {Array.isArray(analysis.matching_methodologies) && analysis.matching_methodologies.length > 0 && (
        <div>
          <h4 className="font-semibold text-surface-900 dark:text-surface-100">Potential matches</h4>
      ...
      {Array.isArray(analysis.gaps) && (
        <div>
          <h4 className="font-semibold text-surface-900 dark:text-surface-100">Gaps</h4>
          <ul className="list-disc pl-5 text-sm">
      ...
      {typeof analysis.recommendation === 'string' && analysis.recommendation.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-950/30 text-blue-800 dark:text-blue-200 p-3 rounded border border-blue-100 dark:border-blue-900 text-sm">
