...
export function CorporateDashboardPage() {
  ...
  const handleGenerateESG = async () => {
    try {
      ...
      showToast(err?.response?.data?.detail || err?.message || 'Failed to generate ESG report')
    }
  }
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="card p-8 text-center">
          <p className="text-red-600 dark:text-red-400 font-medium">Failed to load portfolio</p>
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: 'Credits Held', value: portfolio?.totalHeld?.toLocaleString() ?? '—', sub: 'tCO₂e', icon: Leaf, color: 'text-primary-500' },
          { label: 'Retired', value: portfolio?.totalRetired?.toLocaleString() ?? '—', sub: 'tCO₂e', icon: Shield, color: 'text-primary-500' },
          { label: 'Portfolio Value', value: portfolio ? `$${portfolio.totalValue.toLocaleString()}` : '—', sub: '+12% YTD', icon: FileText, color: 'text-blue-500' },
          { label: 'Projects', value: portfolio?.projects?.length?.toString() ?? '—', sub: 'Active', icon: Eye, color: 'text-violet-500' },
        ].map((stat) => (
              ...
              <div className="card p-5">
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={portfolio?.vintageDistribution ?? []}>
              ...
              <div className="card p-5">
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={portfolio?.methodologyBreakdown ?? []}
                        cx="50%"
                        cy="50%"
                        outerRadius={80}
                        dataKey="value"
                      ...
                      >
                        {(portfolio?.methodologyBreakdown ?? []).map((_, i) => (
          ...
          <div className="card p-6">
            ...
            {generateReport.isError && (
              <div className="mb-4 rounded-lg bg-red-50 dark:bg-red-950/20 p-3 text-sm text-red-600 dark:text-red-400">
                {(generateReport.error as any)?.response?.data?.detail || (generateReport.error as Error)?.message || 'Failed to generate report'}
              </div>
            )}
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={esgForm.companyName}
                  onChange={(e) => setEsgForm({ ...esgForm, companyName: e.target.value })}
                />
              </div>
              <div>
                ...
                <select
                  className="input-modern"
                  value={esgForm.reportingScope}
                  onChange={(e) => setEsgForm({ ...esgForm, reportingScope: e.target.value })}
                >
              ...
              <div>
                ...
                <input
                  type="date"
                  className="input-modern"
                  value={esgForm.periodStart}
                  onChange={(e) => setEsgForm({ ...esgForm, periodStart: e.target.value })}
                />
              </div>
              <div>
                ...
                <input
                  type="date"
                  className="input-modern"
                  value={esgForm.periodEnd}
                  onChange={(e) => setEsgForm({ ...esgForm, periodEnd: e.target.value })}
                />
              </div>
            </div>
          </div>
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                ...
                className="text-sm font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400 flex items-center gap-1.5 transition-colors"
              >
            ...
            <div className="space-y-3">
              {[
                { label: 'Total Offsets Acquired', value: `${portfolio?.totalHeld?.toLocaleString() ?? '—'} tCO₂e` },
                { label: 'Total Retired', value: `${portfolio?.totalRetired?.toLocaleString() ?? '—'} tCO₂e` },
                { label: 'Net Position', value: `${portfolio ? (portfolio.totalHeld - portfolio.totalRetired).toLocaleString() : '—'} tCO₂e` },
              ].map((item) => (
              ...
              <div key={project.name} className="card-hover p-5">
                ...
                <div className="mt-4 space-y-2">
                  {[
                    ...
                    <button
                      key={doc.label}
                      onClick={() => handleDueDiligence(`token-${idx}`)}
                      className="flex w-full items-center justify-between rounded-xl bg-surface-50 dark:bg-surface-800/50 px-4 py-2.5 text-sm text-surface-700 hover:bg-surface-100 dark:text-surface-300 dark:hover:bg-surface-800 transition-colors"
                    >
                      <span>{doc.label}</span>
