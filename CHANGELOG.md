...
- **Pre-Audit Optimization Guide** — Added `docs/PRE_AUDIT_OPTIMIZATION.md` with research on public registry data sources (Verra UI API, Gold Standard authenticated API, CDM pipeline, Carbonmark/VCM.fyi aggregators) and a prioritized roadmap for targeting unaudited projects, incremental document fetching, batch AI evaluation, and change-driven watchlists.
- **Pre-audit pending-only pipeline filter** — Added `PRE_AUDIT_PENDING_ONLY` and `PRE_AUDIT_PENDING_STATUSES` settings. When enabled, the daily `run_pre_audit_pipeline` auto-converts only leads whose registry status is `under_validation`, `under_verification`, or `under_certification`, focusing the workflow on projects where pre-audit saves the most time.
- **Incremental registry document fetch** — Added `etag` and `last_modified` columns to `LeadDocument`. `RegistryDocumentFetcher` now sends `If-None-Match` / `If-Modified-Since` headers and skips S3 upload on `304 Not Modified`, cutting bandwidth and virus-scanning overhead for nightly re-scrapes.
- **Change-driven re-audit for converted projects** — Added `document_fingerprint` to `Lead` and a `compute_document_fingerprint()` helper that hashes each fetched document's source URL and content hash. New Celery task `re_audit_changed_projects` re-queues `run_pre_audit_for_project` only when a converted lead's fetched documents actually change, avoiding redundant AI evaluation and keeping project readiness scores current as registry documents are updated.
- **Pre-audit tasks scheduled in Celery Beat** — `fetch_all_pending_documents`, `run_pre_audit_pipeline`, and `re_audit_changed_projects` are now registered in the daily beat schedule, so document discovery, conversion/pre-audit, and change-driven re-audit run automatically without manual triggers.
